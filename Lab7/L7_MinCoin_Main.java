...
public class L7_MinCoin_Main {
    ...
    static int minNumCoin(int amount, int [] residual) {
        ...
        for (Coins c : Coins.values()) {
            if (c.value() <= amount) {
                // System.out.println("residual[" + (amount - c.value()) + "] = " + residual[amount - c.value()]);
                if (residual[amount -  c.value()] > 0) {// already computed
                    numCoin = 1 + residual[amount - c.value()]; // use it
                } else{
                    numCoin = 1 + minNumCoin(amount - c.value(), residual); // compute it
                }
